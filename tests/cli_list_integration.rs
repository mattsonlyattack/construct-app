...
fn test_list_full_workflow_with_multiple_notes() -> Result<()> {
    ...
    // Verify notes are ordered newest first (descending by created_at)
    assert_eq!(results[0].1, "Fourth note about Rust tutorials");
    assert_eq!(results[1].1, "Third note with no tags");
    assert_eq!(results[2].1, "Second note about learning");
    assert_eq!(results[3].1, "First note about Rust");

    // Verify tag associations
    assert_eq!(results[0].2, vec!["rust", "tutorial"]);
    assert!(results[1].2.is_empty());
    assert_eq!(results[2].2, vec!["learning"]);
...
fn test_list_respects_limit_flag() -> Result<()> {
    ...
    // Verify the 3 most recent notes are returned (newest first)
    assert_eq!(results[0].1, "Note number 5");
    assert_eq!(results[1].1, "Note number 4");
    assert_eq!(results[2].1, "Note number 3");
...
fn test_list_tags_filter_applies_correctly() -> Result<()> {
    ...
    // Verify the correct notes are returned (newest first)
    assert_eq!(results[0].1, "Rust, programming, and tutorial");
    assert_eq!(results[1].1, "Rust and programming");
...
fn test_list_with_combined_short_flags() -> Result<()> {
    ...
    assert_eq!(results[0].1, "Rust note 4");
    assert_eq!(results[1].1, "Rust note 3");
    assert_eq!(results[2].1, "Rust note 2");
